...
export default function GlowingMicButton({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="absolute inset-0 rounded-full pointer-events-none z-0 opacity-70"
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 50% 80%, rgba(59,130,246,0.55) 0%, rgba(59,130,246,0.18) 60%, transparent 100%)",
          filter: "blur(12px)",
        }}
      />
      ...
      <div className="glass-card-less-depth rounded-full w-full h-full flex items-center justify-center cursor-pointer">
        <svg
          ...
          ${isRecording ? "text-white" : "text-white/70 group-hover:text-white"}`}
        >
